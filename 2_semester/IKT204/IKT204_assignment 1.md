...
Figure 1.25 depicts a distributed DoS (DDoS) attack in which the attacker controls multiple sources and directs each source to blast traffic at the target. To cripple the service using this approach, the aggregate traffic rate across all controlled sources must be approximately R. DDoS attacks utilizing botnets with thousands of compromised hosts are common today [DAM 2020]. DDoS attacks are much more difficult to detect and defend against than a single host DoS attack.
